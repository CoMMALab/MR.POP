namespace ppln::collision {
    ...
    )
    {
        ...
        for (int robot_ind=0; robot_ind<ROBOT_COUNT; robot_ind++){
            ...
            for (int j = 0; j < PANDA2_APPROX_JOINT_COUNT; ++j) {
                ...
                if (j==0){
                    for (int c=0; c<4; c++){
                        T_step_col[c] = robot_base_transform_4panda[robot_ind*16 + 4 * c + col_ind];
                    }
                }
                if (j > 0) {
    ...
    template <>
    __device__ bool self_collision_check_approx<ppln::robots::Panda_four>(volatile float* sphere_pos_approx, volatile int* joint_in_collision, volatile int * self_sphere_to_check, const int tid){
    ...
    template <>
    __device__ bool env_collision_check_approx<ppln::robots::Panda_four>(volatile float* sphere_pos_approx, volatile int* joint_in_collision, ppln::collision::Environment<float> *env, volatile int * env_sphere_to_check, const int tid){
        const int thread_ind = tid % 4;
        const int batch_ind = tid / 4;
        bool out = true;
    ...
    // 4 threads per discretized motion for self-collision check
    template <>
    __device__ bool self_collision_check<ppln::robots::Panda_four>(volatile float* sphere_pos, volatile int* joint_in_collision, int self_batch_ind, int thread_interp_cnt, const int tid){
    ...
    // 4 threads per discretized motion for env collision check
    template <>
    __device__ bool env_collision_check<ppln::robots::Panda_four>(volatile float* sphere_pos, volatile int* joint_in_collision, ppln::collision::Environment<float> *env, 
